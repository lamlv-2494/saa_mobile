...
void main() {
  group('Kudos', () {
    ...
    test('awardTitle mặc định là null', () {
      final kudos = createKudos();
      expect(kudos.awardTitle, isNull);
    });

    test('awardTitle nhận đúng giá trị hợp lệ', () {
      final k1 = createKudos(awardTitle: 'IDOL GIỚI TRẺ');
      final k2 = createKudos(awardTitle: 'LEADER');
      final k3 = createKudos(awardTitle: 'HERO');
      expect(k1.awardTitle, 'IDOL GIỚI TRẺ');
      expect(k2.awardTitle, 'LEADER');
      expect(k3.awardTitle, 'HERO');
    });

    test('imageUrls mặc định là danh sách rỗng', () {
      final kudos = createKudos();
      expect(kudos.imageUrls, isEmpty);
    });

    test('imageUrls nhận tối đa 5 ảnh', () {
      final urls = [
      ...
      final kudos = createKudos(imageUrls: urls);
      expect(kudos.imageUrls.length, 5);
      expect(kudos.imageUrls.first, 'https://example.com/1.jpg');
    });

    test('fromJson đọc được awardTitle và imageUrls', () {
      final json = {
        'id': 'k1',
        'sender': {'id': 's1', 'name': 'Sender', 'avatar': '', 'department': '', 'badgeLevel': 0},
        'receiver': {'id': 'r1', 'name': 'Receiver', 'avatar': '', 'department': '', 'badgeLevel': 0},
      ...
      final kudos = Kudos.fromJson(json);
      expect(kudos.awardTitle, 'HERO');
      expect(kudos.imageUrls.length, 2);
    });
  });
}
